...
import {
    ...
    Calendar, DollarSign, Gift, Star, X, ShieldCheck, ShieldAlert, Zap, ShoppingBag, Package, Scale
} from 'lucide-react';
...
const Admin = () => {
    ...
    const handleResolveDispute = async (bountyId, winnerSubmissionId, action) => {
        // action: 'AWARD' or 'REFUND'
        if (!window.confirm(`Force ${action} protocol for this bounty?`)) return;

        try {
            await axios.post(`${API_BASE}/api/bounties/${bountyId}/resolve-dispute`, {
                submissionId: winnerSubmissionId,
                action,
                adminPass: loginData.pass // Simple auth for now
            });
            toast.success("DISPUTE_RESOLVED: Sector stabilized.");
        ...
        <div className="min-h-screen pt-32 pb-20 px-4 max-w-7xl mx-auto space-y-12">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/5 pb-12">
                ...
                <div className="flex gap-4">
                    {['SUPPORT', 'VERIFICATION', 'PAYOUTS', 'PROJECTS', 'BOUNTY_DISPUTES'].includes(activeTab) && (
                        <button onClick={() => {
                            ...
                            if (activeTab === 'BOUNTY_DISPUTES') fetchDisputedBounties();
            ...
            <div className="flex flex-wrap gap-3 mb-10">
                ...
                <button
                    onClick={() => setActiveTab('BOUNTY_DISPUTES')}
                    className={`px-6 py-2.5 rounded-xl font-black uppercase text-[9px] tracking-widest transition-all flex items-center gap-2 ${activeTab === 'BOUNTY_DISPUTES' ? 'bg-red-500 text-white shadow-[0_0_20px_rgba(239,68,68,0.4)]' : 'bg-white/5 text-gray-500 hover:text-white'}`}
                >
                    <Scale className="w-3.5 h-3.5" /> Bounty_Disputes
                </button>
                <button
            ...
            {activeTab === 'BOUNTY_DISPUTES' && (
                <div className="space-y-8">
                    <h2 className="text-2xl font-black text-white uppercase tracking-tighter italic flex items-center gap-3">
                        <Scale className="text-red-500" /> Pending_Dispute_Arbitration
                    </h2>
                    <div className="grid grid-cols-1 gap-6">
                        {disputedBounties.length > 0 ? (
                            disputedBounties.map(bounty => (
                                <GlassCard key={bounty.id} className="border border-red-500/10 hover:border-red-500/30 transition-all">
                                    <div className="flex flex-col md:flex-row justify-between gap-6">
                                        <div className="flex-1 space-y-4">
                                            <div className="flex items-center gap-2">
                                                <span className="bg-red-500/20 text-red-500 px-3 py-1 rounded-lg text-[10px] font-black tracking-widest uppercase">DISPUTED_ZONE</span>
                                                <span className="text-gray-500 font-mono text-[10px]">{bounty.id}</span>
                                            </div>
                                            <h3 className="text-xl font-black text-white uppercase italic">{bounty.title}</h3>
                                            <div className="p-4 bg-void border border-white/5 rounded-2xl">
                                                <p className="text-[10px] text-gray-500 uppercase font-black mb-2">Dispute_Reason</p>
                                                <p className="text-xs font-mono text-red-400">{bounty.disputeReason}</p>
                                            </div>
                                                ...
                                                {(Array.isArray(bounty.submissions) ? bounty.submissions : []).map(sub => (
                                                    <div key={sub.id} className="p-4 bg-white/5 border border-white/5 rounded-2xl relative group">
                                                        <div className="flex items-center gap-2 mb-2">
                                                            ...
                                                            <p className="text-[10px] font-bold text-white">@{sub.username}</p>
                                                            <span className="ml-auto text-[8px] text-gray-500 uppercase font-mono">{sub.status}</span>
                                                        </div>
                                                        <div className="max-h-32 overflow-y-auto text-[10px] font-mono text-gray-400 mb-4 whitespace-pre-wrap">
                                                            {sub.content}
                                                        </div>
                                                        <button
                                                            onClick={() => handleResolveDispute(bounty.id, sub.id, 'AWARD')}
                                                            className="w-full py-2 bg-neon-green/10 text-neon-green border border-neon-green/20 rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-neon-green hover:text-black transition-all"
                                                        >
                                                            Force_Award_to_Sub
                                                        </button>
                                                    </div>
                                        ...
                                        <div className="w-full md:w-64 space-y-4 pt-8 md:pt-0">
                                            <div className="bg-void p-4 rounded-xl border border-white/5 text-center">
                                                <p className="text-[8px] font-black text-gray-500 uppercase">Bounty_Value</p>
                                                <p className="text-2xl font-black text-neon-blue">{bounty.rewardKpc} <span className="text-xs">KPC</span></p>
                                            </div>
                                            <div className="space-y-2">
                                                <button
                                                    onClick={() => handleResolveDispute(bounty.id, null, 'REFUND')}
                                                    className="w-full py-4 bg-red-500/10 text-red-500 border border-red-500/20 rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-red-500 hover:text-white transition-all flex items-center justify-center gap-2"
                                                >
                                                    <RotateCcw className="w-4 h-4" /> Refund_to_Author
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div className="py-20 text-center glass-panel rounded-3xl border border-dashed border-white/5">
                                <Scale className="w-12 h-12 text-gray-800 mx-auto mb-4" />
                                <p className="text-gray-500 font-mono text-xs uppercase tracking-widest">No active disputes requiring arbitration.</p>
                            </div>
                        )}
                    </div>
                </div>
